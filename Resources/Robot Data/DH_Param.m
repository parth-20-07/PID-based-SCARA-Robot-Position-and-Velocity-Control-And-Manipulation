...
l1_dh = [theta1 lb+2*ao l1-2*ao 0]
l2_dh = [theta2 ao l2-ao 0]
l3_dh = [0 -d3+ao ao 0]
...
L(1).qlim = pi/180 * [-90 90];
L(3) =  Link([double(subs(l3_dh,[l1,l2,lb,theta1,theta2,d3,ao],[1,1,1,0,0,1.2,0.05])),1],'standard')
L(3).qlim = [0 2];
scara_robot = SerialLink(L);
scara_robot.name = 'SCARA Robot';
scara_robot.plot([0 0 0],'workspace',[-2 2 -2 2 0 2])
scara_robot.teach